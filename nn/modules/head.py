...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        # =========================================================
        # ✅ 新增：文本残差融合模块（不是 layer，只是 Detect 的私有组件）
        # =========================================================
        
        self._text_feats = None  # 模型级缓存
    ...
    def forward(self, x):
        """
        YOLO Detect head forward with optional text fusion
        ...
        if text_feats is not None:
            # ⚡ 初始化 fusion 模块（只做一次）
            if not hasattr(self, "text_fusion"):
            ...
            # ⚡ 将文本特征融合到每个 feature map
            x = [self.text_fusion(xi, text_feats.to(xi.device)) for xi in x]
            print(f"🔥 Detect.forward fusion applied with text_feats {text_feats.shape}")
        else:
            print("⚠️ _text_feats is None, fusion skipped")

        # YOLO 原始 head forward
