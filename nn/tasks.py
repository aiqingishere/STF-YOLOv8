...
from ultralytics.nn.modules.text_fusion import TextResidualFusion
...
class BaseModel(torch.nn.Module):
    ...
    def predict(self, x, profile=False, visualize=False, augment=False, embed=None):
        ...
        Extended to support text features by caching them as model attributes.
        """
        # -------- 新增：接收 dict batch --------
        if isinstance(x, dict):
            ...
            img = x.get("img", None)
            if img is None:
                raise KeyError("Batch dict must contain key 'img'")

            # 缓存 text features（可能为 None）
            self._text_feats = x.get("text_feats", None)
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        """
        Forward pass with optional text feature fusion for YOLOv8.
        Text features are cached in self._text_feats and consumed inside Detect.forward.
        """
        y, dt, embeddings = [], [], []
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)

        # 取出 BaseModel.predict 缓存的文本特征
        text_feats = getattr(self, "_text_feats", None)

        for m in self.model:
            # -------------------------------
            # 1. 处理 skip connection
            # -------------------------------
            if m.f != -1:
            ...
            # 3. Forward
            # -------------------------------
            # 🔥 对 Detect 模块：内部自行读取 self._text_feats
            x = m(x)
